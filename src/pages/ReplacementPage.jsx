...
export default function ReplacementPage({ coaches, duplicate, onSend, clearDuplicate }) {
  ...
  return <div className="page"><header className="page-header"><div><p className="eyebrow">{duplicate ? 'Nouvelle demande dupliquée' : 'Nouvelle demande'}</p><h1>Créer un remplacement</h1><p>Renseignez le besoin puis choisissez les coachs à contacter.</p></div></header>
    <form onSubmit={submit}><section className="card form-section"><div className="section-title"><span className="step">1</span><div><h2>Le remplacement</h2><p>Informations essentielles de la séance</p></div></div><div className="form-grid">
      ...
      <label>Nom de la gérante<input required value={form.manager_name} onChange={(e) => set('manager_name', e.target.value)} /></label><label>Téléphone de contact<input required type="tel" value={form.manager_phone} onChange={(e) => set('manager_phone', e.target.value)} /></label>
    ...
    <section className="card form-section"><div className="section-title"><span className="step">3</span><div><h2>Aperçu du SMS</h2><p>{message.length} caractères</p></div></div><blockquote>{message}</blockquote><Notice type="info">Le serveur utilise automatiquement le mode test si <code>BREVO_API_KEY</code> est absente.</Notice><Notice type="error">{error}</Notice><div className="submit-row"><span>{selected.length} coach{selected.length > 1 ? 's' : ''} recevra{selected.length > 1 ? 'ont' : ''} ce message</span><button className="button primary" disabled={loading}>{loading ? 'Envoi en cours…' : 'Envoyer la demande'}</button></div></section></form>
  </div>
}
