...
export default function ReplacementDetails({ replacement, onBack, onDuplicate }) {
  ...
  return <div className="page"><button className="back" onClick={onBack}>← Retour à l’historique</button><header className="page-header"><div><p className="eyebrow">Détail de la demande</p><h1>{replacement.class_type} · {replacement.venue}</h1>{replacement.accepted_coach_name && <p className="filled-summary"><strong>Pourvu par {replacement.accepted_coach_name}</strong>{replacement.accepted_at && <> · {new Date(replacement.accepted_at).toLocaleString('fr-FR')}</>}</p>}<span className={`badge ${replacement.status}`}>{statusLabels[replacement.status]}</span></div><button className="button secondary" onClick={() => onDuplicate(replacement)}>Dupliquer</button></header>
    ...
    <section className="card"><div className="section-title"><div><h2>Coachs contactés</h2><p>{replacement.replacement_recipients?.length || 0} destinataires</p></div></div><div className="recipient-list">{replacement.replacement_recipients?.map((r) => <div key={r.id}><span><strong>{r.coach_name_snapshot}</strong><small>{r.phone_snapshot}</small></span><span><em className={`badge ${r.response_status || r.sms_status}`}>{statusLabels[r.response_status || r.sms_status]}</em>{r.sms_status === 'failed' && <small className="danger-text">SMS non envoyé</small>}{r.error_message && <small className="danger-text">{r.error_message}</small>}</span></div>)}</div></section>
  </div>
}
